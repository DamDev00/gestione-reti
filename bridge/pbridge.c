...
int main(int argc, char* argv[]) {
  ...
  if((pd = pcap_open_live(device, snaplen, promisc, 500, errbuf)) == NULL) {
    printf("pcap_open_live -i: %s\n", errbuf);
    return(-1);
   }

   if((pd2 = pcap_open_live(dev2, snaplen, 0, 500, errbuf)) == NULL) {
    printf("pcap_open_live -o: %s\n", errbuf);
    return(-1);
    }


    printf("%s and %s open live!\n", device, dev2);
  ...
  while(1){
    ...
    if(select(maxfd,&readfds,NULL,NULL,NULL)>0){
        if(FD_ISSET(fd_in, &readfds)){
            ...
            printf("arrivato pacchetto!\n");
            int res = pcap_next_ex(pd, &header, &packet);
            if(res>0){
                pcap_sendpacket(pd2, packet, header->len);
            }
        }
        ...
        if(FD_ISSET(fd_out, &readfds)){
            ...
            if(res>0) dummyProcesssPacket(NULL, header, packet);
        }
    }
